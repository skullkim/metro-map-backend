...
export const convertSecond = (second: string = '') => {
  ...
  const tmpResult: string =  (minute >= 60 ?
    convert(minute).from('min').to('h') : minute)
    .toString();
  ...
  if(tmpResult.includes('.')) {
    ...
    const pointBelow = isHour ?
      `${convert(Number(`0.${resultArr[1]}`)).from('h').to('min').toFixed(0)}분`:
      `${convert(Number(`0.${resultArr[1]}`)).from('min').to('s').toFixed(0)}초`;
